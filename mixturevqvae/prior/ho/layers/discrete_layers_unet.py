...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dim % 2 == 1:
            # zero pad
            emb = torch.cat([emb, torch.zeros(emb.size(0), 1).to(x.device)], dim=1)
        return emb
...
class Upsample(nn.Module):
    def __init__(self, dim, output_padding):
        super().__init__()
        self.dim = dim
        padding = 1 if output_padding == 0 else 2

        self.conv = nn.ConvTranspose2d(dim, dim, 4, 2, padding, output_padding=output_padding)
...
class SegmentationUnet(nn.Module):
    def __init__(self, num_classes, dim, num_steps, dim_mults=(1, 2, 4, 8), groups=8, dropout=0.):
        ...
        # output padding for input data with odd size.
        output_padding = 0
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            is_last = ind >= (num_resolutions - 1)
            # add output padding if output size is odd
            if ind >= (num_resolutions - 2) and self.dim % 2 == 1:
                output_padding = 1
            self.ups.append(nn.ModuleList([
                ...
                Upsample(dim_in, output_padding) if not is_last else nn.Identity()
            ]))
    ...
    def forward(self, time, x):
        ...
        for i, (resnet, resnet2, attn, upsample) in enumerate(self.ups):
