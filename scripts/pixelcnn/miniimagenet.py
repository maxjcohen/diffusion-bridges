...
parser.add_argument("--prior-dim", type=int, default=512)
...
class LitPixelCNNTrainer(pl.LightningModule):
    ...
    num_classes = 1
    prior_num_layers = 15
    vqvae_weights_path = "vqvae_miniimagenet.pt"

    def __init__(self, lr=1e-3, prior_dim=256):
        super().__init__()
    ...
    def validation_step(self, images, batch_idx):
        ...
        patch = middle_patch_coordinates(indices.shape[1:], d=9)
...
class Experiment:
    ...
    def __init__(self, args):
        ...
        if args.load_checkpoint is not None:
            self.litmodule = LitPixelCNNTrainer.load_from_checkpoint(args.load_checkpoint)
            print("Loaded litmodule.")
        else:
            self.litmodule = LitPixelCNNTrainer(lr=args.lr, prior_dim=args.prior_dim)
